...
GOUND_TRUTH_PATH="$PATH_2_DATA/test.${TARGET_LANG}-en_XX.en_XX";
...
if [[ "$LANGUAGE_GROUP" == 'one' ]];
then
    lang_pairs="$LANG-en_XX";
    MAX_UPDATE=100000;
    WARMUP=${LANG_WISE_WARMUP[${LANG}]};
    SAVE_DIR=${SAVE_DIR}/${LANG}_en;
    mkdir -p $SAVE_DIR
    train
    evaluate $LANG
else
    ...
    for lang in ${languages[*]}; do
        lang_pairs=$lang_pairs"$lang-en_XX,";
    done
