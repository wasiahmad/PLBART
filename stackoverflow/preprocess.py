...
def extract_7zip():
    with py7zr.SevenZipFile('/local/username/stackoverflow/stackoverflow.com-Posts.7z', mode='r') as z:
        z.extractall()
...
if __name__ == '__main__':
    ...
    extract_7zip()
    split_xml('/local/username/stackoverflow/Posts.xml',
              '/local/username/stackoverflow/xml_shards')
    # convert the xml-style lines into dictionary object
    # ~50M posts are saved in 8 files, each with 6.25M
    txt_to_json('/local/username/stackoverflow/xml_shards',
                '/local/username/stackoverflow/json_shards',
                filesize=6250000)
    # prepare the NL examples
    parse_nl_data('/local/username/stackoverflow/json_shards',
                  '/local/username/stackoverflow/desc_shards')
