...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        # args for "Layer-Wise Coordination between Encoder and Decoder" (He et al., 2018)
        parser.add_argument('--layer-wise-attn', default=False, action='store_true',
        ...
        # fmt: on
        # for T5 like multi-head attention
        parser.add_argument('--rel-pos', action='store_true',
                            help='T5-like relative positional encoding')
        parser.add_argument('--relative-attention-num-buckets', type=int, default=32,
                            help='number of buckets for relative positional encoding')
        parser.add_argument('--max-rel-pos', type=int, default=128,
                            help='maximum distance allowed in relative positional encoding')
        parser.add_argument('--ignore-direction', action='store_true',
                            help='ignore direction in relative attention')
    ...
    def extract_features_scriptable(
        ...
        Returns:
            tuple:
                ...
                - a dictionary with any model-specific outputs
        """
        bsz, query_len = prev_output_tokens.size()
        ...
        if self.rel_pos:
            position_bias = self.compute_bias(query_len, query_len)  # (1, n_heads, qlen, klen)
            position_bias = position_bias.repeat(bsz, 1, 1, 1)
            position_bias = position_bias.view(bsz * self.num_heads, query_len, query_len)
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if encoder_out is not None:
                _encoder_padding_mask = encoder_out.encoder_padding_mask
                if self.layer_wise_attention:
                    _encoder_out = encoder_out.encoder_states[idx]
                else:
