...
@register_task("multilingual_denoising_for_plbart")
class PLDenoisingTask(DenoisingTask):
    @staticmethod
    def add_args(parser):
        ...
        # we use add-lang-token for multilingual denoising
        ...
        parser.add_argument(
            "--extra-langs", type=str, help="language ids we are considering", default=None
        )
        parser.add_argument(
    ...
    def setup_task(cls, args, **kwargs):
        ...
        if args.add_lang_token:
            ...
            if args.extra_langs is not None:
                extra_languages = args.extra_langs.split(",")
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        for language in languages:
            ...
            # NOTE. end_token is a special <LID> symbol
            end_token = (
            ...
            )  # end_token is not used by TokenBlockDataset
            logger.info("loaded {} blocks from: {}".format(len(dataset), split_path))

            # prepend beginning-of-sentence token (<s>, equiv. to [CLS] in BERT)
            dataset = PrependTokenDataset(dataset, self.source_dictionary.bos())
            dataset = AppendTokenDataset(dataset, end_token)  # special symbol <LID>
